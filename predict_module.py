...
class PredictModule:
    ...
    def load_models_and_data(self, model_name, dataset_tag, model_id, from_notebook=True, online=False):
        '''
        ...
        if not online:
            data_path = os.path.join('./datasets/' + model_id, dataset_tag)
        else:
            data_path = os.path.join('./datasets/' + model_id, dataset_tag)

        # Get path and load base model
        try:
            base_model = self.get_base_model(model_name, from_notebook)
        except:
            raise Exception("No base model weights could be located")

        # Get paths and load encoder and similarity models
        network_path = os.path.join(relative, "models", "runs", model_name + dataset_tag, model_id, "encoder_model_weights.h5")
        metricnetwork_path = os.path.join(relative, "models", "runs", model_name + dataset_tag, model_id,
                                          "similarity_model_weights.h5")
        ...
        try:
            print(os.path.join(relative, "models", 'bottlenecks',
                               model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_bottleneck_features_train.npy'))
            print(os.path.join(relative, 'models', 'bottlenecks',
                               model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_train_labels_AWS.npy'))
            gc.collect()
            train_data = np.load(os.path.join(relative, "models", 'bottlenecks',
                                              model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_bottleneck_features_train.npy'))
            gc.collect()
            test_data = np.load(os.path.join(relative, "models", 'bottlenecks',
                                             model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_bottleneck_features_test.npy'))
            gc.collect()
            test_labels = np.array(scipy.sparse.load_npz(os.path.join(relative, "models", 'bottlenecks',
                                                                      model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_test_labels_sparse.npz')).todense())
            train_labels_AWS = np.load(os.path.join(relative, 'models', 'bottlenecks',
                                                    model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_train_labels_AWS.npy'))
        except:
        ...
        train_filenames = np.load(os.path.join(relative, "models", 'bottlenecks',
                                               model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_train_filenames.npy'))

        # Load classes
        classes = np.load(os.path.join(relative, "models", 'bottlenecks',
                                       model_name + '.' + '/' + 'datasets' + '/' + model_id + '/' + dataset_tag + '_CLASSES.npy'))
    ...
    def compute_learned_dist_one_vs_all(self, network, metricnetwork, k, train_data, \
        ...
        for i in range(k):
            gc.collect()
            if predict:
                pairs, actual_categories = self.make_oneshot_task_realistic(train_data, \
                                                                            train_labels, output_labels=1, predict=1, \
                                                                            predict_model_name=predict_model_name,
                                                                            image=image)
            else:
                pairs, targets, actual_categories = self.make_oneshot_task_realistic( \
    ...
    def make_prediction_quad(self, model_name, base_model, network, metricnetwork, image_path, \
        ...
        sorted_distances, sorted_actual_image_index = self.compute_learned_dist_one_vs_all(
                network,
                metricnetwork,
                1,
                train_data,
        ...
        if filename:
            if not online:
                plt.savefig(os.path.join(".", "app", "static", "prediction", "prediction_" + filename + ".png"),
                            dpi=150)
    ...
    def predict(self, image_path, model_id):
        ...
        target_size, \
        test_data, \
        test_labels = self.load_models_and_data(model_name, dataset_tag, model_id, from_notebook, online)
        ...
        gc.collect()
        ##############################################################################
        pred_class, distance, actual_image_index, actual_image_path, matches_list = self.make_prediction_quad(
            ...
            test_labels=test_labels)
        ##############################################################################
        print("++++++", pred_class, distance, actual_image_index, actual_image_path, matches_list)
